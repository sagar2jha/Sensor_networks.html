...
// Call a function repetatively with 5 Second interval getData(); 
...
} }; xhttp.open("GET", "readIR", true); //Handle readDHT server on ESP8266 
xhttp.send(); 
} </script> 
</body> 
</html> 
...
server.on("/readIR", handleIR); //This page is called by java Script AJAX 
server.begin(); //Start server Serial.println("HTTP server started"); 
} void loop(void){ 
